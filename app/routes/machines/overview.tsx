    ...
    <>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col">
          ...
          <p>
            <T text={"Manage the devices connected to your Tailnet."} />{" "}
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <span className="ml-auto text-sm whitespace-nowrap text-mist-500">
          {searchQuery || hasActiveFilters
            ...
            : t(loaderData.populatedNodes.length === 1 ? "{count} machine" : "{count} machines", {
                count: loaderData.populatedNodes.length,
              })}
        </span>
      </div>
          ...
          >
            {filteredAndSortedNodes.length === 0 ? (
              <tr>
                ...
                >
                  <T text={"No machines match the current filters"} />
                </td>
              </tr>
            ) : (
...
export function ErrorBoundary({ error }: Route.ErrorBoundaryProps) {
  const { t } = useI18n();
  return <PageError error={error} page={t("Machines")} />;
}
