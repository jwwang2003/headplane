      ...
      <>
        ...
        <div className="flex h-screen w-screen items-center justify-center bg-black">
          <Card className="w-screen" variant="flat">
            ...
            <Card.Text>
              <Code>{hostname}</Code> <T text={"is not currently connected to the Tailnet."} />
            </Card.Text>
            <Button className="mt-8 w-full" onClick={() => window.location.reload()}>
              <T text={"Retry Connection"} />
            </Button>
          </Card>
        </div>
      </>
...
function BrowserSSHCompatibilityBanner({
  ...
  return (
    <div className="fixed inset-x-4 top-4 z-[60] mx-auto max-w-2xl">
      ...
      >
        <T text={"Headscale 0.29 beta releases through 0.29.1 reject Tailscale's browser/WASM"} />{" "}
        <Code>/ts2021</Code> <T text={"WebSocket request with"} />{" "}
        <Code>405 Method Not Allowed</Code>
        <T text={". Upgrade Headscale to 0.29.2 or newer, or use Headscale 0.28.x."} />
      </StatusBanner>
    </div>
  );
}
