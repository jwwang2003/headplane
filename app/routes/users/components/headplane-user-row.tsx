    ...
    <tr className="group hover:bg-mist-100 dark:hover:bg-mist-800" key={user.id}>
      ...
      <td className="py-2 pl-0.5">
        <p className="text-sm text-mist-600 dark:text-mist-300" suppressHydrationWarning>
          {user.lastLoginAt
            ? new Date(user.lastLoginAt).toLocaleDateString(locale, { timeZone: "UTC" })
            : t("Never")}
        </p>
      </td>
      <td className="py-2 pl-0.5">
...
function mapRoleToName(role: Role) {
  switch (role) {
    ...
    case "admin":
      return <T text="Admin" />;
    case "network_admin":
      return <T text="Network Admin" />;
    case "it_admin":
      return <T text="IT Admin" />;
    case "auditor":
      return <T text="Auditor" />;
    case "viewer":
