...
export default function Page({ loaderData }: Route.ComponentProps) {
  ...
  if (!loaderData.enabled) {
    return (
      <div className="flex max-w-(--breakpoint-lg) flex-col gap-8">
        <Title>
          <T text={"Headplane Agent"} />
        </Title>
        <Notice title={t("Agent Not Enabled")}>
          {loaderData.reason}
          <T text={". To learn how to set up the agent, visit the"} />{" "}
  ...
  return (
    <div className="flex max-w-(--breakpoint-lg) flex-col gap-8">
      ...
      {isPending ? (
        <Notice variant="warning" title={t("Agent Needs Approval")}>
          <T
            text={
      ...
      {loaderData.error ? (
        <Notice variant="error" title={t("Sync Error")}>
          {loaderData.error}
        </Notice>
      ) : undefined}
